...
const BASE_CURATED_UK_CATEGORY_NAMES: Record<string, string> = {
...
// ── Reviewed v4 mappings ─────────────────────────────────────────────────────
// The 72 human-verified source→Ukrainian pairs covering the 73 published
// categories that were still failing closed in production. Kept as its own
// exported table so the reviewed set stays auditable, then merged into
// CURATED_UK_CATEGORY_NAMES below. «Велосипеды» maps once but is used by two
// separate supplier category rows, hence 72 mappings for 73 categories.
export const ADDITIONAL_CURATED_UK_CATEGORY_NAMES: Record<string, string> = {
  'аккумуляторы 12v': 'Акумулятори 12V',
  'аксессуары для лестниц': 'Аксесуари для драбин',
  'алюминиевые лестницы и стремянки': 'Алюмінієві драбини та стрем’янки',
  'бейсболки и банданы': 'Бейсболки та бандани',
  'бензокосы': 'Бензокоси',
  'бензопилы goodluck и другие': 'Бензопили Goodluck та інших брендів',
  'велосипеды': 'Велосипеди',
  'велосипеды-трансформеры': 'Велосипеди-трансформери',
  'воздуходувки': 'Повітродувки',
  'воротки и монтировки': 'Воротки та монтажні лопатки',
  'гироборды и электросамокаты': 'Гіроборди, електроскутери та електросамокати',
  'запчасти': 'Запчастини',
  'запчасти для бензопил': 'Запчастини для бензопил',
  'запчасти для болгарок': 'Запчастини для болгарок',
  'запчасти для велосипеда': 'Запчастини для велосипедів',
  'запчасти для дисковых пил': 'Запчастини для дискових пил',
  'запчасти для лобзиков': 'Запчастини для лобзиків',
  'запчасти для мотокос': 'Запчастини для мотокос',
  'запчасти для мотопомп': 'Запчастини для мотопомп',
  'запчасти для роторных косилок мотоблока': 'Запчастини для роторних косарок мотоблока',
  'запчасти для шуруповертов': 'Запчастини для шуруповертів',
  'запчасти для электропил': 'Запчастини для електропил',
  'конденсаторы': 'Конденсатори',
  'краскопульты': 'Фарбопульти',
  'металлообрабатывающее оборудование': 'Металообробне обладнання',
  'мойки высокого давления': 'Мийки високого тиску',
  'мопеды': 'Мопеди',
  'мото товары': 'Товари для мототехніки',
  'мотоблоки': 'Мотоблоки',
  'мотоботинки probiker': 'Мотоботи PROBIKER',
  'мотокосы бензиновые': 'Бензинові мотокоси',
  'мотоциклы': 'Мотоцикли',
  'мультиметры и тестеры': 'Мультиметри та тестери',
  'мультитулы intertool': 'Мультитули INTERTOOL',
  'насосы и компрессоры': 'Насоси та компресори',
  'очки для мото и спорта': 'Мотоокуляри та спортивні окуляри',
  'пасти для рук': 'Пасти для очищення рук',
  'паяльники': 'Паяльники',
  'перфораторы sds-plus и sds-max с разными режимами работы': 'Перфоратори SDS-Plus та SDS-Max',
  'подшлемники для мотошлемов': 'Підшоломники для мотошоломів',
  'приспособления для авто': 'Пристосування для ремонту авто',
  'пряжки': 'Пряжки',
  'резина, диски и колеса': 'Шини, диски та колеса',
  'ремкомплекты для шин и камер': 'Ремкомплекти для шин і камер',
  'салфетки и губки': 'Серветки та губки',
  'сварочное оборудование': 'Зварювальне обладнання',
  'скотч': 'Клейка стрічка',
  'скутеры': 'Скутери',
  'смазки и масла': 'Мастила та оливи',
  'средства для чистки и ухода': 'Засоби для чищення та догляду',
  'средства индивидуальной защиты': 'Засоби індивідуального захисту',
  'съемники и обжимки': 'Знімачі та обтискачі',
  'триммеры для травы': 'Тримери для трави',
  'упаковочный скотч': 'Пакувальний скотч',
  'фрезеры и наборы фрез': 'Фрезери та набори фрез',
  'фрезы по дереву': 'Фрези для обробки дерева',
  'фэтбайки': 'Фетбайки',
  'хомуты': 'Хомути',
  'шины для детских колясок': 'Шини для дитячих колясок',
  'шины для квадроциклов (atv)': 'Шини для квадроциклів (ATV)',
  'шины для тачек': 'Шини для тачок',
  'шланги высокого давления': 'Шланги високого тиску',
  'шлемы': 'Шоломи',
  'шлемы для скейтбординга': 'Шоломи для скейтбордингу',
  'шлемы-трансформеры': 'Шоломи-трансформери',
  'шуруповерты': 'Шуруповерти',
  'электрокосы и триммеры': 'Електрокоси та тримери',
  'электролобзики': 'Електролобзики',
  'электрооборудование для авто': 'Електрообладнання для автомобілів',
  'электропилы': 'Електропили',
  'электрорубанки': 'Електрорубанки',
  'bmx велосипеды и аксессуары': 'BMX-велосипеди та аксесуари',
}

// The authoritative table: every reviewed mapping, keyed by the lowercased
// normalized source name. Built by spread (not mutation) so it is complete
// before VERIFIED_UK_NAMES below is derived from its values.
export const CURATED_UK_CATEGORY_NAMES: Record<string, string> = {
  ...BASE_CURATED_UK_CATEGORY_NAMES,
