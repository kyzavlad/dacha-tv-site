...
// part of the daily flow. No-op with a clear message when N8N_SEO_WEBHOOK_URL is
// not configured.
export async function GET(req: Request) {
  ...
  const { data: log } = await client
    ...
    .insert({ sync_type: 'category_seo', status: 'running', triggered_by: 'cron' })
    .select('id').single()

  try {
    const result = await sendCategorySeoBatch(100)
    await client.from('supplier_sync_log').update({
      status: result.ok ? 'completed' : 'failed',
      categories_total: result.sent,
