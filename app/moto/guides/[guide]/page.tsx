...
import { getScooterModel, SCOOTER_CATEGORY_SLUG } from '@/lib/moto/scooter-models'
import { displayProductName } from '@/lib/supabase/catalog'
import {
  getScooterGuide,
  SCOOTER_GUIDE_SLUGS,
  type GuideLocale,
...
const UI = {
  uk: {
    ...
    faq: 'Часті запитання',
    products: 'Актуальні товари за темою',
    productsHint: 'Показуємо доступні зараз позиції з каталогу. Перед замовленням звірте модифікацію та конкретну картку товару.',
  },
  ru: {
    home: 'Главная',
    guides: 'Советы',
    ...
    faq: 'Частые вопросы',
    products: 'Актуальные товары по теме',
    productsHint: 'Показываем доступные сейчас позиции из каталога. Перед заказом сверьте модификацию и конкретную карточку товара.',
  },
...
export default async function ScooterGuidePage({ params }: Props) {
  ...
  let topicProducts = [] as Awaited<ReturnType<typeof getInStockScooterModelProducts>>['products']
  if (guide.productDiscovery) {
    const model = getScooterModel(guide.productDiscovery.modelSlug)
    if (model) {
      const mod = guide.productDiscovery.modSlug
        ? model.mods.find((item) => item.slug === guide.productDiscovery?.modSlug)
        : null
      const result = await getInStockScooterModelProducts(
        SCOOTER_CATEGORY_SLUG,
        model.tokens,
        1,
        mod?.tokens,
        guide.productDiscovery.partTokens,
      )
      topicProducts = result.products.slice(0, 8)
    }
  }
  const topicProductListLd = topicProducts.length > 0
    ? itemListSchema(topicProducts.map((product) => ({
        name: displayProductName(product, locale),
        url: localizedPath(locale, `/catalog/${product.category_slug ?? SCOOTER_CATEGORY_SLUG}/${product.slug}`),
      })))
    : null
  ...
  return (
    <main className="min-h-screen bg-cream">
      ...
      {topicProductListLd && <StructuredData data={topicProductListLd} />}
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div className="space-y-10">
            ...
            {topicProducts.length > 0 && (
              <section>
                <h2 className="font-serif text-2xl font-bold text-bark">{t.products}</h2>
                <p className="mt-2 text-sm leading-relaxed text-bark/65">{t.productsHint}</p>
                <div className="mt-5 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                  {topicProducts.map((product) => (
                    <CatalogProductCard
                      key={product.id}
                      product={product}
                      categorySlug={product.category_slug ?? SCOOTER_CATEGORY_SLUG}
                      locale={locale}
                    />
                  ))}
                </div>
