...
import { localizeFlowerVariety } from '@/lib/i18n/manual-attributes'
...
export default async function FlowersPage() {
  ...
  // Group by the canonical Ukrainian DB value; only the visible label is localized.
      ...
      <div className="bg-gray-950 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 md:py-20">
          ...
          <div className="max-w-2xl">
            <p className="text-xs font-semibold text-white/40 uppercase tracking-widest mb-4">{t.flowersEyebrow}</p>
            <h1 className="font-serif text-4xl md:text-5xl font-bold mb-5 leading-tight">{t.flowersH1}</h1>
            ...
            {varietyEntries.length > 1 && (
              <div className="flex flex-wrap gap-2">
                {varietyEntries.map(([variety]) => (
                  ...
                  >
                    {localizeFlowerVariety(variety, locale)}
                  </a>
                ))}
              </div>
            )}
        ...
        {featured.length > 0 && (
          <section className="mb-14">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest">{t.flowersFeatured}</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {featured.map((p) => <FlowerCard key={p.id} product={p} />)}
            </div>
          </section>
        )}
      ...
      <div className="border-t border-gray-100 bg-gray-50">
        <div className="max-w-xl mx-auto px-4 sm:px-6 py-16">
          <div className="text-center mb-8">
            <h2 className="font-serif text-2xl font-bold text-gray-900 mb-2">{t.flowersInquiryTitle}</h2>
            <p className="text-gray-500 text-sm">{t.flowersInquiryBody}</p>
          </div>
