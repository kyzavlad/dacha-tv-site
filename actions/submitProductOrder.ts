...
export async function submitProductOrder(
  ...
  const paymentLabel = PAYMENT_LABELS[d.methodPayment] ?? d.methodPayment
  const itemLines = d.items
    .map((i) => `• ${i.name}${i.variant ? ` (${i.variant})` : ''} × ${i.quantity} — ${i.price * i.quantity} ₴`)
    .join('\n')
  const productSummary = d.items
    .map((i) => `${i.name}${i.variant ? ` (${i.variant})` : ''} × ${i.quantity}`)
    .join(', ')
  const warehouseLabel = d.warehouseName
  ...
  try {
    ...
    // ── Step 3: supplier SKU lookup (decides which items are forwarded) ────────
    ...
    // ── Step 6: supplier forwarding ───────────────────────────────────────────
    // A supplier API failure must NEVER lose the customer order — it is already
    // saved above; we only annotate the supplier_order_* columns with the outcome.
