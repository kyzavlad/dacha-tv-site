...
test('protected cron route keeps explicit-offset pilot/recovery bounded and stateless', () => {
  ...
  assert.match(route, /offset:\s*explicitOffset/)
  assert.match(route, /mode:\s*'manual'/)
  assert.match(route, /batchSize/)
  assert.match(route, /maxBatches/)
  const manualBranch = route.indexOf('if (explicitOffset != null)')
  const durableStateLoad = route.indexOf('state = await loadSyncState(SYNC_TYPE)')
  assert.ok(manualBranch >= 0 && durableStateLoad > manualBranch, 'manual explicit-offset branch must return before durable state is loaded')
