  ...
  if (result?.success) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="font-serif text-xl font-bold text-gray-900 mb-2">{tr({ uk: 'Заявку отримано!', ru: 'Заявка получена!', en: 'Request received!' }, locale)}</h3>
        ...
        >
          {tr({ uk: 'Надіслати ще одну', ru: 'Отправить ещё одну', en: 'Send another one' }, locale)}
        </button>
      </div>
    )
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder={tr({ uk: "Ім'я", ru: 'Имя', en: 'Name' }, locale)}
        />
      </div>

      <div>
        <label htmlFor="flower-phone" className="block text-sm font-semibold text-gray-800 mb-1">
          {tr({ uk: 'Номер телефону', ru: 'Номер телефона', en: 'Phone number' }, locale)} <span className="text-red-500">*</span>
        </label>
        <input
          id="flower-phone"
          name="phone"
          ...
          autoComplete="tel"
          pattern="(\+380|0)\d{9}"
          title={tr({ uk: 'Введіть номер у форматі +380XXXXXXXXX або 0XXXXXXXXX', ru: 'Введите номер в формате +380XXXXXXXXX или 0XXXXXXXXX', en: 'Enter a number in the format +380XXXXXXXXX or 0XXXXXXXXX' }, locale)}
          className="w-full border border-gray-300 rounded-xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-gray-400"
      ...
      {!preselectedProduct && (
        <div>
          <label htmlFor="flower-product" className="block text-sm font-semibold text-gray-800 mb-1">
            {tr({ uk: 'Яка квітка вас цікавить?', ru: 'Какой цветок вас интересует?', en: 'Which flower are you interested in?' }, locale)}
          </label>
          <input
            id="flower-product"
            name="product"
            type="text"
            className="w-full border border-gray-300 rounded-xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-gray-400"
            placeholder={tr({ uk: 'Назва або опис', ru: 'Название или описание', en: 'Name or description' }, locale)}
          />
        </div>
      )}
      ...
      <div>
        <label htmlFor="flower-message" className="block text-sm font-semibold text-gray-800 mb-1">
          {tr({ uk: 'Повідомлення', ru: 'Сообщение', en: 'Message' }, locale)} <span className="text-gray-400 font-normal">{tr({ uk: "(необов'язково)", ru: '(необязательно)', en: '(optional)' }, locale)}</span>
        </label>
        <textarea
          id="flower-message"
          name="message"
          ...
          placeholder={tr({ uk: 'Кількість, термін, побажання...', ru: 'Количество, срок, пожелания...', en: 'Quantity, timing, preferences...' }, locale)}
        />
      </div>
      ...
      >
        {isPending
          ...
          : tr({ uk: 'Надіслати заявку', ru: 'Отправить заявку', en: 'Send request' }, locale)}
      </button>
    </form>
  )
}
